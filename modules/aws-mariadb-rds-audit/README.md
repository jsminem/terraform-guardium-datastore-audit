...
This module uses existing option group with the MariaDB Audit Plugin and existing parameter group.
...
2. `rds-mariadb-parameter-group` - Configures the MariaDB parameter group for audit logging
3. `rds-mariadb-cloudwatch-registration` - Sets up CloudWatch integration for audit logs (when using CloudWatch)
...
| mariadb_major_version | Major version of MariaDB (e.g., '10.6') | string | n/a | yes |
